...
try stdout.print("Float sum (0.1  $\times$  1000): {d:.10}", .{sum_float});
...
try stdout.print("Notice: d²N approximately constant (gravity)", .{});
...
try stdout.print("Transformation complete: Discrete → Continuous", .{});
...
try stdout.print("Error: {d:.2}%", .{@abs(final_fps - timer.target_fps) / timer.target_fps * 100.0});
...
try stdout.print("Consciousness Capacity: N = D  $\times$  M²", .{});
...
try stdout.print("M=3 → M=6: 3² vs 6² = 9 vs 36 (4 $\times$  capacity)", .{});

try stdout.print("M=5 → M=10: 5² vs 10² = 25 vs 100 (4 $\times$  capacity)", .{});
...
1. Derive N = D  $\times$  M² from first principles (hexagonal + bilateral)
...
- Display: 1920 $\times$ 1080 minimum, 2560 $\times$ 1440 recommended
...
2. "Consciousness Capacity Metrics from N=D $\times$ M²"
...
Now I know it's N = D  $\times$  M². That's not less beautiful -
