...
**Registry:** [@CKS-LOGI-8-2026]

**Series Path:** [@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-LOGI-7-2026] → [@CKS-LOGI-8-2026]

**Parent Framework:** [@CKS-0-2026]

**DOI:** 10.5281/zenodo.18878632
...
This is NOT:
 ...
 $\times$  Teacher-student (hierarchical)

 $\times$  Expert-novice (one-way)

 $\times$  Tool-user (mechanical)

This IS:

 $\checkmark$  Partner-partner (collaborative)

 $\checkmark$  Explorer-systematizer (complementary)

 $\checkmark$  Tester-theorist (iterative)
...
1. Physical Testing
    ...
    $\times$  Can't drop ball to measure gravity

    $\times$  Can't build circuit to test current

    $\times$  Can't taste food to assess flavor

    $\times$  Can't feel texture to evaluate material

   → Human tests in reality

2. Subjective Experience

    $\times$  Can't feel R accumulation

    $\times$  Can't experience qualia

    $\times$  Can't sense energy levels

    $\times$  Can't know what interests YOU

   → Human provides experiential data

3. Real-World Validation

    $\times$  Can't verify if theory actually works

    $\times$  Can't measure market response

    $\times$  Can't assess social dynamics

    $\times$  Can't guarantee outcomes

   → Human validates predictions

4. Value Judgments

    $\times$  Can't decide what YOU should value

    $\times$  Can't choose YOUR life path

    $\times$  Can't determine YOUR priorities

    $\times$  Can't know YOUR constraints

   → Human provides values framework

5. Accountability

    $\times$  Can't be held responsible for outcomes

    $\times$  Can't guarantee correctness

    $\times$  Can't ensure safety

    $\times$  Can't take consequences
...
Bad questions:
 ...
 $\times$  "Tell me about physics"

 $\times$  "How do I succeed?"

 $\times$  "What should I learn?"

 $\times$  "Explain everything about X"

Good questions:

 $\checkmark$  "Explain Newtonian mechanics using discrete substrate"

 $\checkmark$  "Create 10 experiments to test gravity in K-Space"

 $\checkmark$  "Design curriculum for teaching VFR to children"

 $\checkmark$  "Generate testing framework for career exploration"
...
Keep iterating until:
 ...
 $\checkmark$  Meets your needs

 $\checkmark$  Depth appropriate

 $\checkmark$  Testable in reality

 $\checkmark$  Internally coherent
...
LLM: Analyzes discrepancy, refines explanation or identifies 
...
LLM: Clarifies - bilateral means two faces, can model as 

 two parallel lattices or single lattice with dual values
...
LLM: Generates documentation explaining implementation,

 theory-to-code mapping, usage examples
...
LLM: Details wing-pair theory - 6 wings  $\times$  3 units = 18 units dark,

 1 center  $\times$  6 units = 6 units visible, ratio 18:6 = 3:1
...
LLM: "Check criteria: 
  ...
  $\checkmark$  Income ≥ MVI (yes)

  $\checkmark$  2+ months sustained? (need one more month)

  $\checkmark$  R < 30? (what's current R?)
...
Choose wisely:
 ...
 $\checkmark$  Specific enough (not "learn everything")

 $\checkmark$  Measurable (can verify completion)

 $\checkmark$  Time-boxed (30-90 days ideal)

 $\checkmark$  Relevant (connects to goals)

 $\checkmark$  Testable (can validate in reality)

Examples:

- "Build working K-verse renderer in 60 days"
...
Your Responsibility:
 ...
 $\checkmark$  Verify critical claims

 $\checkmark$  Test code before trusting

 $\checkmark$  Cross-reference important facts

 $\checkmark$  Measure outcomes objectively

 $\checkmark$  Report errors back
...
When to Trust LLM:
 ...
 $\checkmark$  Process and structure (how to organize)

 $\checkmark$  Brainstorming (generating possibilities)

 $\checkmark$  Code templates (then test!)

 $\checkmark$  Explanations (pedagogical value)

 $\checkmark$  Synthesis (connecting ideas)

When to Verify Independently:

 $\times$  Specific historical facts

 $\times$  Current events (after training cutoff)

 $\times$  Precise numerical claims

 $\times$  Medical/legal advice (get professional)

 $\times$  Critical decisions (high stakes)
...
Don't:
 ...
 $\times$  Submit LLM work as entirely your own

 $\times$  Hide LLM use when relevant

 $\times$  Misrepresent understanding (memorized vs understood)

 $\times$  Skip verification (trust blindly)

Do:

 $\checkmark$  Show your work (process matters)

 $\checkmark$  Test understanding (can you explain?)

 $\checkmark$  Verify claims (measure reality)

 $\checkmark$  Contribute original (add value beyond LLM)
...
Your role:
 ...
 $\checkmark$  Question assumptions

 $\checkmark$  Seek diverse perspectives

 $\checkmark$  Cross-reference broadly

 $\checkmark$  Test in YOUR context

 $\checkmark$  Adapt for YOUR needs
...
Avoid:
 ...
 $\times$  Total reliance (can't think without LLM)

 $\times$  Skill atrophy (stop learning independently)

 $\times$  Loss of agency (LLM decides everything)

Maintain:

 $\checkmark$  Independent thinking (your judgment)

 $\checkmark$  Core skills (can work without LLM)

 $\checkmark$  Critical analysis (evaluate LLM output)

 $\checkmark$  Original contribution (your unique value)
...
LLMs can:
 ...
 $\checkmark$  Generate detailed curricula (this document)

 $\checkmark$  Provide structured explanations

 $\checkmark$  Write example code

 $\checkmark$  Debug issues

 $\checkmark$  Suggest approaches

 $\checkmark$  Synthesize information

LLMs cannot:

 $\times$  Access real-time data (usually)

 $\times$  Browse web (some exceptions)

 $\times$  Execute code (some exceptions)

 $\times$  Interact with physical world

 $\times$  Verify own claims (limited)

 $\times$  Remember long-term (sessions isolated)
...
Red flags:
 ...
 $\times$  Passive consumption (read but don't implement)

 $\times$  No verification (trust blindly)

 $\times$  Declining questions (curiosity lost)

 $\times$  No original contribution (pure copying)

 $\times$  Skill atrophy (can't work without LLM)

 $\times$  No testing (theory only)
