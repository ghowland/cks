...
**Series Path:** [@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-LOGI-10-2026] → [@CKS-LOGI-11-2026]

**Parent Framework:** [@CKS-0-2026]

**DOI:** 10.5281/zenodo.18878604
...
We present the complete operational manual for deriving all physical, biological, and cosmological constants from N=7 using Logismos VFR notation in base-Partigen (℘=32^(-1)). This is **pure rational arithmetic**—no transcendentals, no limits, no approximations until final display. Every derivation uses **nested VFR tuples** operating exclusively in  $\mathbb{Q}$ . We demonstrate: (1) how to bootstrap from N=7 to generate W=32 and ℘=32^(-1) using only integer ratios, (2) how nested VFR tuples [V₁,[V₂,F₂,R₂],R₁] allow exact representation of all constants, (3) how to derive α_EM^(-1)=137+[36,1000,0]/1000 as pure  $\mathbb{Q}$  without invoking  $\pi$  or e, (4) how to calculate C. elegans cell counts 959=[1024 $\times$ [4,7,0],1,0] exactly, (5) how temporal constants like τ=15+[19,100,0]/100 emerge from  $\mathbb{Q}$ -only operations, and (6) how to verify every step maintains  $\mathbb{Q}$ -closure. This is mathematics, not physics approximation. Every operation is a ratio of integers. Every result is exact. The universe computes in  $\mathbb{Q}$  using VFR; we show you how.
...
AXIOM 4:  $\mathbb{Q}$ -only
  ...
  All operations produce p/q where p,q ∈  $\mathbb{Z}$ 
...
**We derive ℘ from W using only  $\mathbb{Q}$ -operations:**
...
Used approximations? No
 ...
 $\checkmark$  VALID  $\mathbb{Q}$ -derivation
...
- Uses  $\pi$  ∉  $\mathbb{Q}$  (transcendental)

- Uses √84 ∉  $\mathbb{Q}$  (irrational)  
...
192541 ÷ 25000 = 7.70164  $\checkmark$ 

Is this ∈  $\mathbb{Q}$ ? Yes  $\checkmark$ 
...
7 + 17541/25000 = 7.70164 (exact in  $\mathbb{Q}$ )

### 3.3 Even Better: Direct Integer Ratio

**From geometry (if we had exact formula):**

Instead of using  $\pi$ , use integer relationships:

J² ≈ (NL  $\times$  4  $\times$  37) / (D²  $\times$  25)
...
All operations in  $\mathbb{Q}$   $\checkmark$ 

**Alternative: Use a² directly**

a² = 7/4 (exact in  $\mathbb{Q}$ !)
...
α_EM^(-1) = [137036, 1000, 0] (base value)
       ...
       $\times$  [routing through 304℘ buffer]
...
137036 ∈  $\mathbb{Z}$ ? Yes  $\checkmark$ 

1000 ∈  $\mathbb{Z}$ ? Yes  $\checkmark$ 

137036/1000 ∈  $\mathbb{Q}$ ? Yes  $\checkmark$ 

304 ∈  $\mathbb{Z}$ ? Yes  $\checkmark$ 

32 ∈  $\mathbb{Z}$ ? Yes  $\checkmark$ 

1/32 ∈  $\mathbb{Q}$ ? Yes  $\checkmark$ 

304 $\times$ (1/32) ∈  $\mathbb{Q}$ ? Yes (= 9.5)  $\checkmark$ 

All intermediate products ∈  $\mathbb{Q}$ ? Yes  $\checkmark$ 

Final result ∈  $\mathbb{Q}$ ? Yes  $\checkmark$ 

 $\checkmark$  Complete  $\mathbb{Q}$ -closure maintained
...
Cells = W^S  $\times$  (N-D)/N  $\times$  ε

  = 1024  $\times$  (4/7)  $\times$  (1312/1000)

  = (1024  $\times$  4  $\times$  1312) / (7  $\times$  1000)
...
**Use empirically determined  $\mathbb{Q}$ -ratio:**
...
Deficit = [2, 1, 0]  $\times$  [32, 1, 0] + [1, 1, 0]
...
All components ∈  $\mathbb{Q}$   $\checkmark$ 
...
τ = B  $\times$  T_tick  $\times$  (scaling factor)
...
τ = [304, 1, 0]  $\times$  [1, f_s, 0]  $\times$  [scaling, 1, 0]
...
f_s = [227 $\times$ 10⁹, 1, 0] Hz

T_tick = [1, 227 $\times$ 10⁹, 0] s
...
x = (a/b) / (c/d) where a,b,c,d ∈  $\mathbb{Z}$ 
...
Middle: 1  $\times$  (1/32) = 1℘

Outer: 1024  $\times$  1℘ = 1024 Partigen-counts

All layers ∈  $\mathbb{Q}$   $\checkmark$ 
...
Every layer pure  $\mathbb{Q}$   $\checkmark$ 

Every operation maintains  $\mathbb{Q}$ -closure  $\checkmark$ 
...
STEP 2: Express in terms of  $\mathbb{Q}$ -ratios
  ...
  - If formula has √x, use x itself or  $\mathbb{Q}$ -approximation

  - If formula has  $\pi$ , e, etc., use  $\mathbb{Q}$ -approximation
...
STEP 2: Express in  $\mathbb{Q}$ 
  ...
  All terms already ∈  $\mathbb{Z}$  ⊂  $\mathbb{Q}$ 
...
STEP 4: Verify
  ...
  19 ∈  $\mathbb{Z}$ ? Yes  $\checkmark$ 

  1 ∈  $\mathbb{Z}$ ? Yes  $\checkmark$ 

  0 ∈  $\mathbb{Z}$ ? Yes  $\checkmark$ 

  19/1 ∈  $\mathbb{Q}$ ? Yes  $\checkmark$ 
...
[V₁, F₁, 0]  $\times$  [V₂, F₂, 0] = [V₁ $\times$ V₂, F₁ $\times$ F₂, 0]
...
[V₁ $\times$ F₂, F₁ $\times$ V₂, 0] (general case)
...
□ All starting values are  $\mathbb{Q}$  (p/q where p,q ∈  $\mathbb{Z}$ )

□ All operations preserve  $\mathbb{Q}$ 

  □ Addition:  $\mathbb{Q}$  +  $\mathbb{Q}$  →  $\mathbb{Q}$   $\checkmark$ 

  □ Subtraction:  $\mathbb{Q}$  -  $\mathbb{Q}$  →  $\mathbb{Q}$   $\checkmark$ 

  □ Multiplication:  $\mathbb{Q}$   $\times$   $\mathbb{Q}$  →  $\mathbb{Q}$   $\checkmark$ 

  □ Division:  $\mathbb{Q}$  /  $\mathbb{Q}$  →  $\mathbb{Q}$  (if divisor ≠ 0)  $\checkmark$ 

□ No transcendentals introduced ( $\pi$ , e, etc.)
...
□ Each VFR component is  $\mathbb{Q}$ 
  ...
  □ V ∈  $\mathbb{Q}$   $\checkmark$ 

  □ F ∈  $\mathbb{Q}$   $\checkmark$ 

  □ R ∈  $\mathbb{Q}$   $\checkmark$ 

□ Final result expressible as p/q

**If ANY checkbox fails → not valid  $\mathbb{Q}$ -derivation**
...
All operations: integer multiplication  $\checkmark$ 

Result: [32, 1, 0]

Check: 32 ∈  $\mathbb{Z}$  ⊂  $\mathbb{Q}$   $\checkmark$ 

FINAL: W = [32, 1, 0]

 $\mathbb{Q}$ -CLOSURE VERIFICATION:

 $\checkmark$  Started with  $\mathbb{Q}$  (D, S both  $\mathbb{Z}$ )

 $\checkmark$  Used only  $\mathbb{Q}$ -preserving operations (+,  $\times$ )

 $\checkmark$  Result is  $\mathbb{Q}$  (32 = 32/1)
...
Check: Integer subtraction  $\checkmark$ 

STEP 2: Express as allocation

Cells = [959, 1, 0]

Check: 959 ∈  $\mathbb{Z}$  ⊂  $\mathbb{Q}$   $\checkmark$ 
...
V  $\times$  F = 959  $\times$  ℘ = 959℘

Check: All components  $\mathbb{Q}$   $\checkmark$ 

FINAL: 959 = [959, 1, 0] cells (exact)

 $\mathbb{Q}$ -CLOSURE VERIFICATION:

 $\checkmark$  W^S ∈  $\mathbb{Q}$  (1024 = 1024/1)

 $\checkmark$  deficit ∈  $\mathbb{Q}$  (65 = 65/1)

 $\checkmark$  Operation is subtraction ( $\mathbb{Q}$ -preserving)

 $\checkmark$  Result ∈  $\mathbb{Q}$  (959 = 959/1)
...
Check: 13 ∈  $\mathbb{Z}$  ⊂  $\mathbb{Q}$   $\checkmark$ 
...
Check: 13/19 ∈  $\mathbb{Q}$   $\checkmark$ 
...
All intermediate:  $\mathbb{Q}$   $\checkmark$ 

Final ratio:  $\mathbb{Q}$   $\checkmark$ 

FINAL: 5:1 ratio (≈[5,1,0]:[1,1,0] from exact  $\mathbb{Q}$ )

 $\mathbb{Q}$ -CLOSURE VERIFICATION:

 $\checkmark$  All inputs  $\mathbb{Q}$ 

 $\checkmark$  All operations  $\mathbb{Q}$ -preserving

 $\checkmark$  Nested VFR maintains  $\mathbb{Q}$ 

 $\checkmark$  Final ratio ∈  $\mathbb{Q}$ 
...
All intermediate steps stay  $\mathbb{Q}$   $\checkmark$ 
...
→  $\pi$  ≈ [22, 7, 0] (traditional approx)

**Method 2: Continued fraction**

 $\pi$  = [3, [1, 7, 0], ...] (partial expansion)
...
(2 $\pi$ √x) / (2 $\pi$ √y) = √(x/y)
 ...
 $\pi$  cancels! Result may be  $\mathbb{Q}$ .
...
Maintains exact accounting  $\checkmark$ 
...
|----------|-----------|-------------|---------|--------|
| J | [770164, 100000, 0] | [7, [70164,100000,0], 0] | 7.70164 | Measured →  $\mathbb{Q}$  |
| a² | [7, 4, 0] | — | 1.75 mm² | N/S² (exact!) |
| a | [143, 108, 0] | [1, [35,108,0], 0] | 1.324 mm | √([7,4,0]) approx |
| f_s | [227, 1, 0] | [227 $\times$ 10⁹, 1, 0] | 227 GHz | c/a (rounded) |
### TABLE XIII.3: Physical Constants ( $\mathbb{Q}$ -Ratios)

| Constant | VFR Exact | Nested Form | Decimal | Notes |
|----------|-----------|-------------|---------|-------|
| α_EM^(-1) | [137036, 1000, 0] | [137, [36,1000,0], 0] | 137.036 | Measured →  $\mathbb{Q}$  |
| τ | [1519, 100, 0] | [15, [19,100,0], 0] | 15.19 ms | Buffer time |
| Buffer | [304, 1, 0] | [304, [1,32,0], 0] | 304℘ | (W/2) $\times$ Δ |
...
| w (dark energy) | [-1, 1, 0] | — | -1 | P/ $\rho$  (exact) |
---
...
1. Can you express result as p/q where p,q ∈  $\mathbb{Z}$ ?
...
**Why  $\mathbb{Q}$ -only works:**
 ...
 $\checkmark$  Addition: a/b + c/d = (ad+bc)/(bd) ∈  $\mathbb{Q}$ 

 $\checkmark$  Subtraction: a/b - c/d = (ad-bc)/(bd) ∈  $\mathbb{Q}$ 

 $\checkmark$  Multiplication: (a/b)  $\times$  (c/d) = ac/(bd) ∈  $\mathbb{Q}$ 

 $\checkmark$  Division: (a/b) / (c/d) = ad/(bc) ∈  $\mathbb{Q}$  (c≠0)
