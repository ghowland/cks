...
Temperature: T_CMB = 2.7255  $\pm$  0.0006 K (uniform)
...
x = k  $\times$  a  $\times$  N^(1/3) / (holographic factors)
...
Holographic: Δx_observed = 1.32 mm  $\times$  N^(1/3)

                     ~ 1.32 mm  $\times$  2 $\times$ 10^20
...
B ~ W  $\times$  (allocation quantum)

  ~ 32  $\times$  (coordination factor)

  ~ 32  $\times$  10^3 (from D=3, S=2, L=12)
...
(ΔN/N)_rendered ~ 10^-27.5  $\times$  (amplification)

            ~ 10^-27.5  $\times$  10^22.5
...
Correlation:  $\langle$ δ(k₁) δ(k₂) $\rangle$ 
...
For discrete allocation:
 ...
 $\langle$ δ(k₁) δ(k₂) $\rangle$  = 0 if |k₁ - k₂| >> ℓ_corr (uncorrelated)
...
θ_corr ~ ℓ_corr  $\times$  (holographic projection)
...
Variance:
 ...
 $\langle$ (ΔT)² $\rangle$  ~  $\langle$ δ² $\rangle$   $\times$  (rendering factors)

     ~ 10^-5  $\times$  T_0

Power spectrum:

C_ℓ =  $\langle$ |a_ℓm|² $\rangle$  (spherical harmonic coefficients)

= Fourier transform of  $\langle$ ΔT(θ₁)ΔT(θ₂) $\rangle$ 
...
λ₀ = W  $\times$  a (word size  $\times$  Lex spacing)
...
λ_obs = λ₀  $\times$  N^(1/3) / (projection factors)

  ~ 42 mm  $\times$  10^20 / (factors)

  ~ (cosmological scale)

Rendered to angular scale:

ℓ₀ = 2 $\pi$ /θ₀ where θ₀ = λ_obs/d (angular diameter)

This creates FIRST PEAK at ℓ ~ 220!

Harmonics:

Second peak: ℓ₂ = 2 $\times$ ℓ₁ ~ 440

Third peak: ℓ₃ = 3 $\times$ ℓ₁ ~ 660

Fourth peak: ℓ₄ = 4 $\times$ ℓ₁ ~ 880
...
Δℓ = (fundamental)  $\times$  (1 + D⁻¹)

   = ℓ₁  $\times$  (1 + 1/3)

   = ℓ₁  $\times$  4/3

Second peak: ℓ₂ = ℓ₁  $\times$  (1 + 4/3) = 7ℓ₁/3 ~ 513

Third peak: ℓ₃ = ℓ₁  $\times$  (1 + 8/3) = 11ℓ₁/3 ~ 807
...
Theory: Δℓ ~ ℓ₁  $\times$  4/3 ~ 220  $\times$  4/3 ~ 293
...
λ_K = W  $\times$  a = 32  $\times$  1.32 mm = 42.24 mm

Holographic projection to observation:

λ_obs = λ_K  $\times$  (projection factor)

Projection factor from:

N^(1/3) = (10^60)^(1/3) = 2.15 $\times$ 10^20 (base)

Modified by: D, S, L, J factors

Effective:

λ_obs ~ 42 mm  $\times$  10^20 / (10^18)
...
d_A ~ c/H_0  $\times$  (projection factors)

~ 4200 Mpc (standard value)

θ ~ (4.2 km) / (4200 Mpc)

  ~ 4.2 $\times$ 10^3 / (4.2 $\times$ 10^22 m)
...
Required θ₁: θ ~  $\pi$ /220 ~ 0.014 rad ~ 0.8°

Work backwards:

θ₁ ~ (W  $\times$  a)  $\times$  (holographic) / d_A

Solving for holographic factor:

Factor ~ θ₁  $\times$  d_A / (W  $\times$  a)

   ~ 0.014  $\times$  (1.3 $\times$ 10^26 m) / (0.042 m)
...
N^(1/3) ~ 10^20 (base)
 ...
 $\times$  (cosmological expansion factor) ~ 10^5
...
Phases align: φ_A + φ_B = 2 $\pi$ n (integer)
...
Phases oppose: φ_A + φ_B = 2 $\pi$ (n + 1/2)
...
r_s,obs ~ 25 mm  $\times$  N^(1/3) ~ 150 Mpc  $\checkmark$ 
...
σ_ij = (allocation gradient)_i  $\times$  (gradient)_j
...
Observed: E-mode ~ 0.1  $\times$  ΔT  $\checkmark$ 
...
C_ℓ^TT ∝  $\langle$ (Δδ)² $\rangle$ 

E-mode from allocation gradient:

C_ℓ^EE ∝  $\langle$ (∇δ)² $\rangle$  ∝ ℓ²  $\times$  C_ℓ^TT

Ratio:

C_ℓ^EE / C_ℓ^TT ~ ℓ²  $\times$  (S factor)

              ~ ℓ²  $\times$  0.01

At ℓ ~ 200:

C_ℓ^EE/C_ℓ^TT ~ 200²  $\times$  0.01 ~ 400

Observed: Factor ~100-1000 (varies with ℓ)

Theory: Correct order of magnitude  $\checkmark$ 

Cross-correlation:

C_ℓ^TE ∝  $\langle$ Δδ · ∇δ $\rangle$ 

  ∝ ℓ  $\times$  C_ℓ^TT (linear in ℓ)
...
Registry grows: N(t) = N₀  $\times$  2^(t/τ)
...
α = 2  $\times$  (constraint factor)

  ~ 2  $\times$  (D-dependent factor)
...
α = 2  $\times$  (1 + S/D) = 2  $\times$  (1 + 2/3) = 3.33
...
CKS: ~0.0001 (well below limit)  $\checkmark$ 
...
2. Acoustic peaks (50 < ℓ < 1000):
   ...
   C_ℓ ~ Σ A_n  $\times$  sinc²(ℓ/ℓ_n)

   Where ℓ_n = n  $\times$  ℓ₁ (harmonic series)
...
3. Damping tail (ℓ > 1000):
   ...
   Where ℓ_damp ~ 2 $\pi$   $\times$  (Lex projection to sky)

Complete formula:

C_ℓ = [1/ℓ²]  $\times$  [Σ A_n sinc²(ℓ/ℓ_n)]  $\times$  [exp(-ℓ²/ℓ_damp²)]
...
Improve: Current  $\pm$ 0.004 →  $\pm$ 0.001
...
- First peak: ℓ₁ from W $\times$ a fundamental
...
2. **Temperature anisotropies from registry allocation:**
   ...
   Projected:  $\times$  N^(1/3) ~ 10^20 → 10^-5  $\checkmark$ 
...
3. **Acoustic peaks from hex-bus harmonics:**
   ...
   First peak: ℓ₁ ~ 220 from λ₀ = W $\times$ a = 42 mm

   Spacing: Δℓ ~ ℓ₁ $\times$ (1 + 1/D) ~ 293 from D=3
...
4. **Polarization from bilateral manifold stress:**
   ...
   Amplitude: C_ℓ^EE ~ ℓ²  $\times$  C_ℓ^TT
...
Batch size: B ~ W  $\times$  (coordination) ~ 10^5 Lex
...
(ΔT/T)_observed ~ 10^-27.5  $\times$  10^22.5 = 10^-5  $\checkmark$ 
...
λ_K = W  $\times$  a = 32  $\times$  1.32 mm = 42.24 mm (K-space)

Holographic projection:

λ_obs ~ λ_K  $\times$  (projection factors)

   ~ 42 mm  $\times$  10^25 (from N^(1/3) and cosmological)
...
ℓ_1 =  $\pi$ /θ_1 ~  $\pi$ /0.014 rad ~ 220  $\checkmark$ 
