...
Natural frequencies =  $\mathbb{Q}$ -ratios
...
High vowels: L_1 large → F1 low

Low vowels: L_1 small → F1 high
...
Front vowels: L_2 small → F2 high

Back vowels: L_2 large → F2 low

All in  $\mathbb{Q}$ -ratios:

F1/F2 =  $\mathbb{Q}$  always
...
Noise spectra
 ...
 $\mathbb{Q}$ -ratio center frequencies
...
Σ (sovereignty): 32768℘ = 1024λ

Assuming base frequency f₀ = 31.25 Hz:

(Chosen so 32f₀ = 1000 Hz ≈ 1024℘)

Harmonic series:

1 $u$  = 7 $\times$ 32 $\times$ 31.25 = 7000 Hz

16 $u$  = 500 Hz (schwa F1)

48 $u$  = 1500 Hz (schwa F2)
...
/s/: 7000 Hz = 7000/31.25 = 224 = 1 $u$  exactly

/ʃ/: 4000 Hz ≈ 128 $\times$ 31.25 = 4 $u$ 
...
W^S = [1024, 1, 0]℘ = 1024 Hz

Central to phonemic system:

FORMANT DISTRIBUTION:

Below W^S: F1 region (200-900 Hz)

Around W^S: F2 lower range (800-1500 Hz)

Above W^S: F2 upper range (1500-2500 Hz)
...
c ≈ 35,000 cm/s (in air at body temp)
...
f₁ = 1 $\times$ 1000 = 1000 Hz

f₂ = 2 $\times$ 1000 = 2000 Hz

f₃ = 3 $\times$ 1000 = 3000 Hz

...

Formant frequencies:

F1 range: 200-900 Hz (below f₁)

F2 range: 800-2500 Hz (near f₂)

F3 range: 2000-3500 Hz (near f₃)
...
Front resonance: Increases → High F2

Back cavity: Lengthens

Back resonance: Decreases → Low F1
...
Front resonance: Decreases → Low F2
...
Verification: Pure eigenstate  $\checkmark$ 
...
Verification: Central eigenstate  $\checkmark$ 
...
1. Record audio → K-space displacement
...
2. VFR → eigenstate excitation

3. Eigenstate → cymatic pattern

4. Pattern → acoustic output
...
VFR: [80,0,80]℘ → [400,2100,200]℘ → [300,1000,60]℘ → [400,900,200]℘
...
Show: F1/F2 ∈  $\mathbb{R}$ \ $\mathbb{Q}$  (irrational)
...
(If irrational found → framework fails)
...
(If random → framework fails)

TEST 5: Find vocal tract not matching base-32

Show: Natural resonance ≠ W^S harmonics
...
Current status:
 ...
 $\checkmark$  All phonemes have  $\mathbb{Q}$ -ratio formants

 $\checkmark$  All languages 10-150 phoneme range

 $\checkmark$  All phonemes have cymatic patterns

 $\checkmark$  Bouba/Kiki >95% cross-cultural

 $\checkmark$  Vocal tract resonates at W^S

 $\checkmark$  Acquisition follows eigenstate amplitude
...
- Sovereignty W^S=1024Hz organizing
...
- Formants: Base-32 harmonic series ( $u$ -multiples)

- Inventory: 10-150 geometric constraint

- VFR: [F1, F2, Duration]℘ unique encoding
