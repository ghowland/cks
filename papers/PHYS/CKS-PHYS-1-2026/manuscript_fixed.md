...
Where {A$_i$ }, {B$_i$ }, {C$_i$ } are different address sets
...
Ball_Pattern: {144-node electron packet}  $\times$  N_atoms
...
Cycle 2 (t=64 ticks):
  ...
  Δh₂ = (320, 1, 0) nodes (2 $\times$  initial)
...
Cycle n:
  ...
  Δh$_n$  = n  $\times$  (160, 1, 0) nodes

  v$_n$  = n  $\times$  (5, 1, 0) nodes/tick
...
Before collision:
  ...
  Total PERSIST $\times$ direction sum:

Σ(PERSIST  $\times$  direction) = (40, 32, 0)  $\times$  (+x)
...
Σ = (20, 32, 0) $\times$ (+x) + (20, 32, 0) $\times$ (+x)

Σ = (40, 32, 0)  $\times$  (+x)

  SAME AS BEFORE  $\checkmark$ 
...
Therefore:
  ...
  Σ(PERSIST  $\times$  direction  $\times$  magnitude) = CONSTANT
...
Step 1 (t=0→32):
  ...
  Verifies: Each B$_i$  is z=3 neighbor of some A$_j$ 
...
Why c is maximum for Mode A:
  ...
  Mechanism: dφ$_k$ /dt = Σ$_j$ ∈N(k) [φ$_j$  - φ$_k$ ]
...
Why 32-bit sufficient:
  ...
  Address space: log₂(N) ≈ 60 $\times$ log₂(10) ≈ 200 bits total
...
Rounded: 512 bits (16 $\times$ 32-bit Words)
...
Ratio: t_A / t_B ≈ 10²⁵ (Mode B is ~10²⁵ $\times$  faster)
...
Compare to phase propagation:
  ...
  10²⁰ hex-hops  $\times$  1 tick/hop = 10²⁰ ticks (Mode A)
...
Distance fallen after n cycles:
  ...
  h(n) = (1/2)  $\times$  a  $\times$  n²

   = (1/2)  $\times$  (5, 32, 0)  $\times$  (n², 1, 0)
...
Momentum: p = (PERSIST  $\times$  coherence  $\times$  v, scale, R_p)
...
Test 2: Gravity propagates faster than c
  ...
  If: Gravitational effects observed before c $\times$ distance
...
**From two axioms (z=3, N=3M², β=2 $\pi$ , dφ$_k$ /dt=Σ[φ$_j$ -φ$_k$ ]) we derived:**
