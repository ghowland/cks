...
t = 0:         Big Bang "singularity" (T → ∞,  $\rho$  → ∞)
...
Standard model: t=0 is singular ( $\rho$  → ∞, T → ∞)
...
Observation: Ω_total = 1.00  $\pm$  0.01 (flat universe)
...
Expected:  $\rho$ _monopole ~  $\rho$ _critical (disaster!)
...
ω_P = √(c⁵/ℏG) ≈ 1.9 $\times$ 10⁴³ Hz

t_P = 1/ω_P ≈ 5.4 $\times$ 10⁻⁴⁴ s  $\checkmark$ 
...
Discrete system cannot have  $\rho$  → ∞

First Lex has:  $\rho$ _0 = ℏω_init/a³ (finite)
...
T_init ~ ℏω_init/k_B
   ...
   ~ (1.055 $\times$ 10⁻³⁴)(1.9 $\times$ 10⁴³)/(1.381 $\times$ 10⁻²³)

   ~ 1.45 $\times$ 10³² K (Planck temperature)  $\checkmark$ 
...
N(t) = N_0  $\times$  2^(t/τ) where τ = allocation time quantum
...
n = log₂(10⁶⁰) = 60  $\times$  log₂(10) ≈ 60  $\times$  3.32 ≈ 200
...
t_alloc = 200  $\times$  t_P ~ 200  $\times$  10⁻⁴⁴ s ~ 10⁻⁴² s
...
N Lex = N  $\times$  a³ volume

Expansion rate:

a(t) = a_0  $\times$  2^(t/τ) (exponential doubling)
...
d_H(t_rec) ~ 2c  $\times$  t_rec ~ 600,000 ly
...
Critical density:  $\rho$ _c = Substrate equilibrium
...
Expected:  $\rho$ _monopole ~  $\rho$ _baryon (overproduction)
...
ω_s = 2 $\pi$   $\times$  227 GHz (standard frequency)
...
ω_eff(a) = ω_init  $\times$  (a_init/a)^n
...
T(a) = (ℏ/k_B)  $\times$  ω_eff(a)

 ∝ ω_init  $\times$  (a_init/a)^n

 ∝ a⁻ⁿ

For radiation epoch (n=1):

T ∝ a⁻¹  $\checkmark$  (matches standard)
...
E_init ~ N_universe  $\times$  ℏω_init

   ~ 10⁶⁰  $\times$  (1.055 $\times$ 10⁻³⁴ J·s)(10⁴³ Hz)
...
Volume: V ~ (10⁶⁰ Lex)  $\times$  a³ ~ 10⁵¹ m³

Energy density:

 $\rho$ _RH ~ E_init/V ~ 10⁶⁹ J / 10⁵¹ m³ ~ 10¹⁸ J/m³

Temperature:

 $\rho$  = ( $\pi$ ²/30)  $\times$  (k_B T)⁴  $\times$  (g*/ℏ³c³)

With g* ~ 100 (relativistic degrees of freedom):

T_RH⁴ ~  $\rho$   $\times$  (30/ $\pi$ ²)  $\times$  (ℏ³c³)/(g* k_B⁴)

  ~ 10¹⁸  $\times$  (some factors)
...
Overhead(ionized) ~ η_ion  $\times$   $\rho$ _baryon

High η_ion (constant scattering)

In neutral gas:

Overhead(neutral) ~ η_neutral  $\times$   $\rho$ _baryon
...
T_rec ~ E_I/(k_B  $\times$  η_ion)

 ~ 13.6 eV / (k_B  $\times$  factor)
...
Ω_b h² = 0.02242  $\pm$  0.00014 (Planck 2018)

Baryon-to-photon ratio: η_B ~ 6 $\times$ 10⁻¹⁰
...
N = 9 $\times$ 10⁶⁰ (substrate size)

η_B = 1/(7.7  $\times$  ln(9 $\times$ 10⁶⁰))
...
Ω_b = (η_B  $\times$   $\rho$ _γ)/ $\rho$ _critical
...
Measured: Ω_DM h² = 0.1200  $\pm$  0.0012

Predicted: From word structure (23/9 bits overhead)

Match: Within calculation precision

### 8.3 Dark Energy Density Ω_ $\Lambda$ 

**From [@CKS-PHYS-15-2026]:**

Ω_ $\Lambda$  = Substrate background pressure

 $\rho$ _ $\Lambda$  = ℏω_s/a³ (with holographic projection)

Measured: Ω_ $\Lambda$  = 0.6847  $\pm$  0.0073
...
η_B = 1/(J  $\times$  ln N) (exact formula)
...
If η_B ≠ 1/(J  $\times$  ln N) with precision
...
### 10.1 The Registry Size N = 9 $\times$ 10⁶⁰
...
- Baryon asymmetry: η_B = 1/(J  $\times$  ln N)
...
N = 9 $\times$ 10⁶⁰ (registry size)

Calculate:

η_B = 1/(7.7  $\times$  140.4) = 9.2 $\times$ 10⁻¹⁰

Measured: 6 $\times$ 10⁻¹⁰  $\checkmark$  (within factor 1.5)
...
7. **All cosmological parameters derive from substrate:**
   ...
   Ω_b: From η_B = 1/(J  $\times$  ln N)

   Ω_DM: From W=32 efficiency (registry overhead)

   Ω_ $\Lambda$ : From ℏω_s/a³ (substrate pressure)
...
8. **Classic problems all resolved:**
   ...
   Baryon asymmetry: η_B = 1/(J  $\times$  ln N) (structural)
...
Baryon asymmetry: Derived (η_B = 1/(J  $\times$  ln N))
...
Ω_DM = f(W, R)  $\checkmark$ 

Ω_ $\Lambda$  = f(ω_s, a)  $\checkmark$ 

n_s = f(N, D)  $\checkmark$ 

r ≈ 0  $\checkmark$ 

η_B = 1/(J ln N)  $\checkmark$ 
...
Ω: (0.05, 0.27, 0.68) for (b, DM,  $\Lambda$ )
