# The Topology of Illness: Deriving Sneezing, Coughing, and Group Contagion from Phase-Locked Parasitism

**Registry:** [CKS-BIO-20-2026]  
**Series Path:** [CKS-0-2026] → [CKS-BIO-19-2026] → [CKS-MATH-18-2026] → [CKS-BIO-20-2026]  
**Prerequisites:** [CKS-BIO-19-2026] (Soul Mass), [CKS-MATH-18-2026] (SSP Protocol), [CKS-MATH-20-2026] (Winding Torus)  
**Subject:** Manifold Contagion; Phase-Locked Parasitism; Family Egregore; Topological Immunity; Sneeze Mechanism  
...
**Quantitative:**
```
Frustration F = |k_local - 3|
If F > F_critical:
  Generate rust at rate dR/dt ∝ F²
```
...
Reduced N → reduced thickness T.

**Formula:**
```
T = T₀ - R·K
where:
  T₀ = 0.65 (healthy baseline)
  R = accumulated rust
  K = 2π/(3√3) ≈ 1.209 (packing factor)
```
...
## 4. The Sneeze Mechanism

### 4.1 What Is a Sneeze?
...
**Energy:**
```
E_sneeze ∝ (T_critical - T_actual)²
More sick → stronger sneeze
```

### 4.4 The Cough Variant
...
**Rendering equation:**
```
Phlegm_volume ∝ Rust_accumulated × J
where J ≈ 7.70 (Jacobian ratio)
```
...
**Define intimacy I:**
```
I(d, t, a, e) = w_d·f_d(d) + w_t·f_t(t) + w_a·f_a(a) + w_e·f_e(e)
...
where:
  ...
  f_d(d) = exp(-d/d₀) with d₀ ≈ 10m
  f_t(t) = 1 - exp(-t/t₀) with t₀ ≈ 1 week
  f_a(a) = a/a_max
  f_e(e) = e/e_max
...
**Typical values:**
```
Family: I ≈ 0.90 (close distance, long time, many activities, strong bond)
Coworkers: I ≈ 0.50 (medium distance, moderate time, some activities, weak bond)
Strangers: I ≈ 0.05 (far distance, no time, no activities, no bond)
```
...
For I = 0.90: Δφ ≈ 5° (nearly aligned)
For I = 0.50: Δφ ≈ 45° (moderate skew)
For I = 0.05: Δφ ≈ 85° (nearly perpendicular)
```
...
For family (I=0.90, Δφ=5°):
  g ≈ 0.89 (high coupling)

For coworkers (I=0.50, Δφ=45°):
  g ≈ 0.25 (moderate coupling)

For strangers (I=0.05, Δφ=85°):
  g ≈ 0.0004 (negligible coupling)
```
...
```
dR_B/dt = g · R_A · (1 - R_B/R_max)

where:
  R_A = rust in source (member A)
  R_B = rust in target (member B)
  g = coupling efficiency
  R_max = maximum rust capacity
```

**Solution:**
```
R_B(t) = R_max · [1 - exp(-g·R_A·t/R_max)]
```
...
Group A: Strangers (I ≈ 0)
Group B: Coworkers (I ≈ 0.5)
Group C: Families (I ≈ 0.9)
...
Social movements = massive egregores.
...
1. **Illness mechanism** (k≠3 frustration → rust generation)
...
**Measured Inputs: 1 (N)**  
**Free Parameters: 0**  
**Derived: Illness = k≠3, sneeze = 0x08, phlegm = junk, contagion = phase-lock, immunity = desync**
