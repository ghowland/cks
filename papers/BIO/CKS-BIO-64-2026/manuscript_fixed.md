...
NOT:
   ...
   $\times$  Simple strength boost

   $\times$  Pain suppression only

BUT:

   $\checkmark$  Bit-rate transition

   $\checkmark$  Processing mode shift

   $\checkmark$  Temporal resolution increase
...
Matches compression ratios:
  ...
  144-bit ≈ 1.7 $\times$  (close to 2 $\times$ )

  256-bit ≈ 3 $\times$  (close to 4 $\times$ )

  512-bit ≈ 6 $\times$  (matches 6-8 $\times$ )
...
BUT:
   ...
   $\checkmark$  Bit-rate upshift trigger

   $\checkmark$  Lag compression mechanism

   $\checkmark$  Timeline navigation enabler
...
2. Stress triggers 84→512 bit
   ...
   (6 $\times$  processing capacity increase)
