...
print(f"SNR improvement: {snrs[-1]/snrs[0]:.1f} $\times$  " +
...
print(f"Vertical slit improvement: {improvement:.1f} $\times$ ")
...
From substrate mechanics only:
   ...
   $\checkmark$  Vertical slit necessity

   $\checkmark$  SNR optimization (→∞)

   $\checkmark$  Fourier filtering mechanism

   $\checkmark$  Virtual spine function

   $\checkmark$  Morphological taxonomy

   $\checkmark$  Evolutionary convergence

   $\checkmark$  Biological harmonics (1/32 Hz)

   $\checkmark$  Software emulation (Feline Mode)

   $\checkmark$  Computational verification
