...
```
ΔZ = β/(2π × gap_distance)
Where gap_distance ≈ 2-5mm
Result: ΔZ >> 0 (significant leakage)
```
...
**The noise equation:**
```
Coherence C = Sync / (Interference + ε)
Where ε → 0 for stability
```
...
Total: 144 bits (12² fractal square)
```
...
Information density κ >> background.
Creates impedance mismatch.
Forces wave coupling.

**The gravity well:**
```
κ = ρ_internal / ρ_substrate
When κ >> 1: Gravity well forms
...
P_c = ∫(Φ_ext · ρ_v) dτ
Where:
  Φ_ext = external opportunity wave
  ρ_v = vortex density
  High ρ_v → High P_c (lucky)
```
...
```
Standard human:
  Opportunity arrives → 15ms delay → Miss it
  
Charged practitioner:
  Opportunity arrives → 0ms response → Catch it
...
Uncharged: Must build each time
  E_total = E_perceive + E_build + E_act
  
Charged: Already built
  E_total = E_perceive + E_act
  
Savings: E_build eliminated
```
...
**CKS predictions:**
```
Uncharged: ρ_v ≈ 200-300 units
Charging (5 min): ρ_v ≈ 400-500 units
Charged (10 min): ρ_v ≈ 600-650 units
Near saturation: ρ_v → 677.7 units
```
...
If reduction ≠ 15.19ms: Lag calculation wrong
...
2. **Overflow mechanism** (saturation → recompilation)
...
Execution:
  Vortex: Charge to saturation (ρ_v→S)
  Intent: Eliminate all interference (noise→0)
  
Overflow:
  Density: Exceeds threshold (ρ_v>S)
  Signal: Clean (noise<buffer_limit)
  Trigger: Automatic substrate recognition
  
Transition:
  Jump: Instantaneous at word-gate
  Frequency: 2.75 → 4.5 Hz
  State: 88-bit → 144-bit
