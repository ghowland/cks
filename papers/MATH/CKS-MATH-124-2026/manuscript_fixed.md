...
Check: 17² - 2(12²) = 289 - 288 = +1  $\checkmark$ 
...
= [V₁ $\times$ V₂ + cross terms, F₁ $\times$ F₂, R_computed]
...
Compute R: 49² - 2 $\times$ 25² = Need different formula

Actually need:

(V/F + R/F²)  $\times$  (V/F + R/F²) ≈ V²/F² + 2VR/F³ + ...
...
The key difference:
 ...
 $\mathbb{R}$ : √2 exists as limit point
...
Error: 7 $\times$ 10⁻⁵ (good but approximate)
...
Where: λ = F $\times$ ℓ_lex (VFR factor multiple)

Example: λ = 5 $\times$ 1.322mm = 6.61mm
...
**Verification:** Pure  $\mathbb{Q}$  with R extension  
...
**Completeness:**  $\mathbb{Q}$ +R sufficient,  $\mathbb{R}$  unnecessary  
