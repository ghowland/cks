...
**Series Path:** [@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-MATH-86-2026] → [@CKS-MATH-87-2026]

**Parent Framework:** [@CKS-0-2026]

**DOI:** 10.5281/zenodo.18878872
...
For a point P = [x₀ : ... : x$_n$ ] ∈ V( $\mathbb{Q}$ ) in projective space, define:
...
- **b** = number of irreducible components of effective cone  $\Lambda$ _eff(V)
...
**Projective space ℙⁿ( $\mathbb{Q}$ ):**

Points are equivalence classes:

P = [x₀ : x₁ : ... : x$_n$ ]

where x$_i$  ∈  $\mathbb{Q}$  and not all zero, with [x] ~ [λx] for λ ∈  $\mathbb{Q}$ *.

**Normalization:**

Write in lowest terms:

x$_i$  = a$_i$ /d where gcd(a₀, ..., a$_n$ , d) = 1
...
V( $\mathbb{Q}$ ) = {P ∈ ℙⁿ( $\mathbb{Q}$ ) : P satisfies all f$_i$  = 0}
...
#{(x, y, z) ∈  $\mathbb{Z}$ ³ : x² + y² + z² ≤ R²} ~ (4 $\pi$ /3)R³
...
ℙⁿ: Pic(ℙⁿ) ≅  $\mathbb{Z}$  (generated by hyperplane), a = 1

ℙ¹  $\times$  ℙ¹: Pic ≅  $\mathbb{Z}$ ² (two line classes), a = 2

Cubic surface: Pic ≅  $\mathbb{Z}$ ⁷, a = 7

### 4.2 The Effective Cone

**Definition:**

The **effective cone**  $\Lambda$ _eff(V) ⊂ Pic(V) ⊗  $\mathbb{R}$  consists of classes of effective divisors:

 $\Lambda$ _eff(V) = {[D] : D ≥ 0 is an effective divisor}

**Properties:**

- Convex cone in Pic(V) ⊗  $\mathbb{R}$  ≅  $\mathbb{R}$ ^a
...
b = number of irreducible components of  $\Lambda$ _eff(V)
...
N(V, H) = (density)  $\times$  (volume in a dimensions)  $\times$  (boundary corrections)
...
V( $\mathbb{Q}$ ) ↔ integral solutions to torsor equations

**Height on torsor:**

H_ $\mathcal{T}$  ~ H^(1/deg(-K_V))

**Counting integral points:**

#{integral points on  $\mathcal{T}$  with H_ $\mathcal{T}$  ≤ T}

~ (geometric factor) · T^(dim  $\mathcal{T}$ )

**Dimension of  $\mathcal{T}$ :**

dim  $\mathcal{T}$  = dim V + a (adding Picard rank)
...
N(ℙⁿ, H) = #{[x₀:...:x$_n$ ] : gcd=1, max|x$_i$ |≤H}

     ≈ H (primitive points in  $\mathbb{Z}$ ⁿ⁺¹)
...
N(V( $\mathbb{R}$ ), H) ~ (volume of V) · H^dim(V)

**But points are in  $\mathbb{Q}$ :**

N(V( $\mathbb{Q}$ ), H) ~ c · H^a · (log H)^(b-1)
...
c_V = (hexagonal packing factor)  $\times$  (anticanonical integral)
...
**Question:** Does Manin hold over  $\mathbb{Q}$ (√d)?
...
N(V, H) = (density c_V)  $\times$  (volume H^a)  $\times$  (boundary corrections (log H)^(b-1))
