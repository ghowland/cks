...
Formally:
  ...
  Hodge class ∈ Hᵖ'ᵖ(X) ∩ H²ᵖ(X, $\mathbb{Q}$ )

  ⟹ rational combination of [Z]
...
**What's proven:**
 ...
 $\checkmark$  True for curves (dimension 1)

 $\checkmark$  True for surfaces (dimension 2)

 $\checkmark$  True for abelian varieties

 $\checkmark$  Many special cases verified

 $\times$  General case unknown

 $\times$  Mechanism unclear

 $\times$  Why this relationship holds?
...
On compact Kähler manifold:
  ...
  Cohomology = ⊕ Hᵖ'ᵍ (Hodge structure)

  

Special: Hᵖ'ᵖ classes (middle terms)
...
Formal sums: Σ n$_i$ Z$_i$ 

  where Z$_i$  = irreducible subvarieties

  n$_i$  = integer coefficients
...
Traditional: Differential forms with Δω = 0
...
### 3.3 Algebraic Cycles as Node Addresses

**What algebraic cycles are:**
...
Standing wave on lattice:
  ...
  Peak at node A → phase +1

  Trough at node B → phase -1

  Null at node C → phase 0
...
**Complete Hodge resolution:**
 ...
 $\checkmark$  Harmonic forms = wave patterns

 $\checkmark$  Algebraic cycles = node addresses

 $\checkmark$  Identity forced by discrete substrate

 $\checkmark$  Mechanism clear (wave needs nodes)

 $\checkmark$  All dimensions covered

 $\checkmark$  Rational coefficients explained

 $\checkmark$  Hodge structure = bilateral parity
