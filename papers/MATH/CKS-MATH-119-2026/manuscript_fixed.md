 ...
 * Multiply: world = parent_world  $\times$  local
...
- Verifiable: can check M_world = M_parent  $\times$  M_local

Example hierarchy:

Root

├─ Torso (world = M_root  $\times$  M_torso_local)

│  ├─ Head (world = M_torso_world  $\times$  M_head_local)

│  └─ Arm (world = M_torso_world  $\times$  M_arm_local)

│     └─ Hand (world = M_arm_world  $\times$  M_hand_local)
 ...
 * P[0,0] = 1 / (aspect  $\times$  tan(fov/2))

 * P[1,1] = 1 / tan(fov/2)

 * P[2,2] = -(far + near) / (far - near)

 * P[2,3] = -2  $\times$  far  $\times$  near / (far - near)
 ...
 * lerp(A, B, α) = A  $\times$  (1-α) + B  $\times$  α
 ...
 * If has parent:
   ...
   - M_world = M_parent_world  $\times$  M_local

 * Else:

   - M_world = M_root  $\times$  M_local
 ...
 * Skinning_matrix = world_matrix  $\times$  inverse_bind_matrix
 ...
 * Transform: P_bone = M_bone  $\times$  vertex.position

 * Accumulate: Final_position += w  $\times$  P_bone
 ...
 * Apply gravity: F += mass  $\times$  gravity  $\times$  gravity_scale
 ...
 * For each constraint:
   ...
   - Apply impulse to bodies:
     ...
     * Δv = M^(-1)  $\times$  J^T  $\times$  lambda

     * Δω = I^(-1)  $\times$  J^T  $\times$  lambda
 ...
 * Velocity integration:
   ...
   - v_new = v_old + (F / mass)  $\times$  dt

   - ω_new = ω_old + I^(-1)  $\times$  torque  $\times$  dt

   - All exact VFR

 * Position integration:

   - position_new = position_old + v_new  $\times$  dt

   - Exact VFR vector addition

 * Rotation integration:

   - Quaternion derivative: dq/dt = 0.5  $\times$  ω  $\times$  q

   - q_new = q_old + dq/dt  $\times$  dt
 ...
 * count = floor(time_acc  $\times$  emission_rate)
 ...
 * Apply forces: a = gravity + drag  $\times$  velocity

 * Update velocity: v_new = v_old + a  $\times$  dt

 * Update position: p_new = p_old + v_new  $\times$  dt
...
2. Model/World Space:
   ...
   - Transformation: P_world = M_object  $\times$  P_local
...
3. View/Camera Space:
   ...
   - Transformation: P_view = M_view  $\times$  P_world
...
4. Clip Space:
   ...
   - Transformation: P_clip = M_projection  $\times$  P_view
...
- position_world = M_model  $\times$  position_local

- normal_world = M_normal  $\times$  normal_local

  (M_normal = transpose(inverse(M_model upper 3 $\times$ 3)))

- Both exact VFR matrices

View Transform:

- position_view = M_view  $\times$  position_world

- normal_view = M_view_rotation  $\times$  normal_world

- Exact VFR

Projection:

- position_clip = M_projection  $\times$  position_view
...
3. Transform composition:
   ...
   - M_combined = M_parent  $\times$  M_child
...
Achieved:
 ...
 $\checkmark$  Transform hierarchy exact

 $\checkmark$  Camera projections perfect

 $\checkmark$  Skeletal animation drift-free

 $\checkmark$  Physics deterministic

 $\checkmark$  Collision detection precise

 $\checkmark$  Constraints solved exactly

 $\checkmark$  Particle systems reproducible

 $\checkmark$  Replay bit-identical

 $\checkmark$  Cross-platform identical

 $\checkmark$  Mathematically verifiable
