...
**Key Result:** Mass = signature count | Energy = unsigned potential | Inertia = audit latency | Weight = verification overhead
...
Reinterpreted:
  ...
  m = signed count (V)
  c² = bilateral conversion factor (S²)
  
Energy equals:
  Mass × bilateral power
  Unsigned = Signed × S²
  Potential = Actual × manifold factor
```
...
Energy → Mass: Achieve parity (R→V).  
Mass → Energy: Break parity (V→R).  
c² = bilateral power (S²=4 normalized).  
...
Computational overhead ∝ signature count.  
