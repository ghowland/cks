...
where a₁, a₂, ..., a$_k$ , b are positive integers, then:
...
Sum: 31,846,943,392,197,857  $\checkmark$ 
...
The maximum power n for which k$_m$ $_i$ $_n$  < n is:
...
a₁ⁿ + ... + a$_n$ ₋₁ⁿ = bⁿ mostly has NO solutions
...
This should be ~64 $\times$  faster than brute force because Step 1 eliminates 63/64 of candidates.
...
S(n, k, N) = c(n,k)  $\times$  N^(k/n) / (S·W)^(n-k) + O(...)
