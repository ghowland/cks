...
Rare (special contexts):
  ...
  57 (D $\times$ Δ): Error correction sums
...
Why 8 $\times$ 8 specifically?

Historical evolution:

  Early variants: 5 $\times$ 5, 6 $\times$ 6, 10 $\times$ 10
...
SU(3) symmetry:
  ...
  3 $\times$ 3 matrix representation
...
Sudoku: 9 $\times$ 9 with 3 $\times$ 3 blocks
...
Magic squares: 3 $\times$ 3 minimal
...
12 periodic table groups:
  ...
  Matches D $\times$ S^S = 12

   $\checkmark$  Partially confirmed
...
After measurement:
   ...
   $\checkmark$  9 gluons (8+1) found

   $\checkmark$  Rubber optimized at 19-20

   $\checkmark$  Critical nodes ~1024 connections
...
NOT:
   ...
   $\times$  Independent discoveries

   $\times$  Empirical accidents

   $\times$  Domain-specific

   $\times$  Requiring separate explanation

  

BUT:

   $\checkmark$  Forced combinations

   $\checkmark$  From 3 parameters only

   $\checkmark$  Cross-domain unity

   $\checkmark$  Single geometric substrate
