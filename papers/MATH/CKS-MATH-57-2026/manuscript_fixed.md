...
Example:
  ...
  Search P ∈ [1,10]  $\times$  [1,10]

  100 candidates (2 $\times$ 2 matrix)

  Each evaluation O(n²)

  Total O(100n²) vs O(k $\times$ n³) iterations
...
Scan:
  ...
  m candidates  $\times$  n² evaluation
...
**Complete DARE solution:**
 ...
 $\checkmark$  Inversion eliminated (bilateral mesh)

 $\checkmark$  Remainder minimization (mod-32)

 $\checkmark$  Integer optimal state (discrete P)

 $\checkmark$  Automatic gradient (thermodynamic)

 $\checkmark$  Zero jitter control (exact commands)

 $\checkmark$  Singularity-proof (no failures)

 $\checkmark$  O(1) snap complexity (vs O(n³) iteration)

 $\checkmark$  Industrial applicability (guidance/control)
...
Scan m candidates  $\times$  n² operations.  

vs k iterations  $\times$  n³ operations.  
