...
Normal directions: (2, 1, 0) (±n̂)

Therefore: S = (2, 1, 0) mandatory

Alternative S values:
  S = 1: Möbius strip (non-orientable, χ≠2)
...
Result: S must be integer ✓
        S must equal 2 (from above) ✓
```
...
```
Single side: E = φ × c (linear)
Both sides: E = φ × c × c = φ × c²
...
Combined: c × c = c² propagation constraint
...
2D commit: Area (length × width) → c² = c^S where S=2
...
Combined: (E_total, 1, R_t) = E_A × E_B ∝ c × c = c²
...
Step 1: Single-sided energy
  E_1 = base_energy × c (propagation constraint)
...
This is F=ma:
  Force needed ∝ mass × acceleration
...
Energy per side: (Δv × c, 1, R_e)
Number of sides: S = (2, 1, 0)
Total energy: (Δv × c², 1, R_t)
...
Given:
  - S=2 (forced by topology)
  - c=1 (forced by discrete lattice)
  - β=2π (forced by phase conservation)
