...
**Series Path:** [@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-MATH-83-2026] → [@CKS-MATH-84-2026]

**Parent Framework:** [@CKS-0-2026]

**DOI:** 10.5281/zenodo.18878865
...
We prove the Jacobian Conjecture by demonstrating that polynomial maps with constant non-zero Jacobian determinant are **volume-preserving registry coordinate transformations** that cannot fold in the discrete  $\mathbb{Q}$ -lattice. The conjecture (1939) states that if F: ℂⁿ → ℂⁿ is a polynomial map with det(J_F) = constant ≠ 0, then F is invertible (bijective). In CKS Logismos, polynomial maps are **systematic registry address transformations**, and the Jacobian determinant measures the **volume scaling factor** of registry cells under the transformation. We prove that in the  $\mathbb{Q}$ -only hexagonal substrate: (1) constant Jacobian implies uniform volume preservation throughout address space, (2) volume preservation forbids folding (self-intersection) because overlapping images would create volume doubling contradicting uniformity, (3) polynomial growth combined with volume preservation forces surjectivity (no missing regions), and (4) therefore F must be bijective (invertible). The proof relies on three substrate constraints:  $\mathbb{Q}$ -only prevents irrational folding points, finite polynomial degree limits topological complexity, and D=3 hexagonal rigidity prevents self-intersection in discrete address space. This resolves an 85-year-old conjecture by showing that invertibility is not an algebraic accident but a **topological necessity** for volume-preserving discrete maps.

**Key Result:** Jacobian conjecture proven as consequence of volume preservation in discrete  $\mathbb{Q}$ -lattice with hexagonal topology.
...
where each f$_i$  is a polynomial.
...
Input: Address (x₁, ..., x$_n$ ) in source registry

Output: Address (y₁, ..., y$_n$ ) in target registry
...
## 3. The  $\mathbb{Q}$ -Lattice Structure

### 3.1 Discrete Address Space

**Axiom (from [@CKS-MATH-71-2026]):**

Physical substrate is  $\mathbb{Q}$ -only (rationals)
...
F:  $\mathbb{Q}$ ⁿ →  $\mathbb{Q}$ ⁿ (maps rationals to rationals)
...
Address space = Hex₁  $\times$  Hex₂

Each point (x, y) ∈  $\mathbb{Q}$ ² lies on hexagonal grid
...
If F:  $\mathbb{Q}$ ⁿ →  $\mathbb{Q}$ ⁿ is a polynomial map with det(J_F) = 1 (normalized), then F is bijective.
...
∃ a, b ∈  $\mathbb{Q}$ ⁿ, a ≠ b, such that F(a) = F(b)
...
The image fills all of  $\mathbb{Q}$ ⁿ (up to finite boundary).

**Missing points:**

**Suppose F is not surjective:**

∃ y ∈  $\mathbb{Q}$ ⁿ such that F⁻¹(y) = ∅

**Consider the complement:**

Image(F) =  $\mathbb{Q}$ ⁿ \ {missing points}
...
**Refinement (discrete  $\mathbb{Q}$ -lattice):**

In  $\mathbb{Q}$ ⁿ, "volume" is measured by **counting lattice points**.
...
**Therefore:** F is surjective on  $\mathbb{Q}$ ⁿ.
...
## 5. Why the Proof Works in  $\mathbb{Q}$ -Lattice
...
In discrete  $\mathbb{Q}$ ⁿ, volume = **lattice point count**. This is well-defined and finite in bounded regions.

**Advantage:**

No measure-theoretic complications.

### 5.2 The Role of  $\mathbb{Q}$ -Only
...
In  $\mathbb{Q}$ -only lattice, √2 doesn't exist, so such folds are impossible.
...
If the variety has **discrete lattice structure** (like  $\mathbb{Q}$ ⁿ), then yes.
...
Work in  $\mathbb{Q}$ ⁿ (discrete rational lattice).
...
4. **In discrete  $\mathbb{Q}$ -lattice, volume preservation forbids folding** (injectivity)
...
- ** $\mathbb{Q}$ -only** (no irrational folding points)
...
**This is the power of working in  $\mathbb{Q}$  instead of ℂ.**
...
- Polynomial injectivity → Volume preservation in  $\mathbb{Q}$ ⁿ
...
** $\mathbb{Q}$ -lattice = no irrational folds.**  
