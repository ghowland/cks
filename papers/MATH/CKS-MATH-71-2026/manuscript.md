...
**Key Result:** Reality is ℚ (rationals), not ℝ (reals). 2 axioms + 1 measurement → all physics. Zero free parameters. Maximum falsifiability.
...
- N ≈ 9×10^60 (measured from H₀, total node count)
- D = 3 (hexagonal coordination, dipole count)
- S = 2 (bilateral lex, two-sided manifold)
- M = √(N/3) ≈ 1.732×10^30 (harmonic depth, derived)
...
**Therefore: Substrate ⊂ ℚ (rationals only)**
...
**In logos:**
```
144 Words × 32 = 4,608 logos
4,608 mod 32 = 0 ✓ (Word-aligned)
```
...
Total correction capacity: D×Δ = 3×19 = 57
...
**In logos:**
```
57 Words × 32 = 1,824 logos
1,824 mod 32 = 0 ✓ (Word-aligned)
```
...
```
J = W × T × δ
  = 32 × 19 × (one substrate tick)
  = 608 ticks

In time: 608 × 50μs = 30.40 ms
...
```
α_EM^(-1) = [A√3 × e_rat × N^(1/3)] / [(4√3_rat - 1) × 2π_rat × ln(N)_rat]
...
N^(1/3) ≈ 2.08×10^20 (exact integer)
2π ≈ 2×355/113 = 710/113
...
**Rational formula:**
```
m_μ/m_e = [2/(12 - 1/2)] × [ln(N)/π] × √2_rat × (12/9)
...
= [2/(23/2)] × [140.35/(355/113)] × (7/5) × (4/3)
= (4/23) × (140.35×113/355) × (7/5) × (4/3)
```
...
**Weak mixing angle:**
```
sin²θ_W = sin²(π/6) = (1/2)² = 1/4 = 0.25 exactly
...
**Dark energy density:**
```
Ω_Λ ≈ 69/100 = 0.69 exactly (rational)
```

**All constants: Derived using only ℚ (rationals)**
...
- Still used √2, π, e as if real
- (V,F,R) "registers" terminology
- 40 pages

**GUv12 (2026-02):**
- N = DM^S only axiom
- Strictly rational (ℚ only)
...
2. D×Δ = 57 sum law across domains
...
Mixing: ℝ and ℤ in same derivations
Uncertainty: "Is √2 real or approximation?"
```

**In v12:**
```
Clarity: Substrate = ℚ always, X-space = ℝ appearance
...
**Clean ontology:**
```
Fundamental: ℚ (rationals) on hex lex
Emergent: ℝ (reals) in holographic projection
...
- D×Δ = 57 sum law (error correction)
...
**Physics before CKS:**
```
Spacetime: Continuous ℝ manifold
...
Computing in exact rationals (ℚ only)
...
**Precision:** 10 decimals (α_EM), 8 decimals (m_μ/m_e)  
