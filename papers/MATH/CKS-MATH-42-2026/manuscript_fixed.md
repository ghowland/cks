...
Prime p₁: Leans toward Side A

Prime p₂: Leans toward Side B
...
Need one from each side:
  ...
  Side A contribution: p₁

  Side B contribution: p₂
...
For primes:
  ...
  Tension spike at p₁ (Side A)

  Requires counter-spike at p₂ (Side B)
...
ax2.set_xlabel('Prime p₁ (Side A)', color='gray')

ax2.set_ylabel('Prime p₂ (Side B)', color='gray')
...
if pairs:
    ...
    ax4.plot([0, p1], [0, 1], 'cyan', linewidth=3, label=f'p₁={p1} (Side A)')
...
But made from:
  ...
  One tilts left (prime p₁)

  One tilts right (prime p₂)
...
Goldbach conjecture is bilateral tension cancellation law. Even numbers are balanced states requiring decomposition into opposing asymmetries. Primes are fundamental tension spikes (indivisible by D=3, S=2, L=32) that lean toward one manifold side. From S=2 structure, balanced states must split into contributions from both sides. For even 2n, midpoint n defines bilateral axis. Any displacement x creates symmetric positions n-x and n+x. These must both be prime (fundamental spikes) to sum to even 2n (balanced state). Universal construction guaranteed because 144-163-19 triad saturates registry with prime remainders at all scales. Not probabilistic search but geometric necessity—mirror law ensures for any tension spike, reciprocal recoil exists on opposite side. Every straight line (even) is two crooked sticks (primes) leaning together.
