...
Example: 50 GHz = 1.5625  $\times$  10⁹  $\times$  32 Hz
...
Substrate: Tick-rate sampling (fs = 32 Hz  $\times$  N)
...
Substrate: fs = 32 Hz  $\times$  1536 = 49.152 kHz
...
y[n] = Σ h[k]  $\times$  x[n-k]

Integer implementation:

y[n] = (Σ H[k]  $\times$  x[n-k]) / SCALE

Where H[k] = round(h[k]  $\times$  SCALE)
...
Hex FFT: O(N log_6 N) ≈ O(N  $\times$  0.56 log N)
...
- LED array: 40 Hz flicker ( $\pm$ 1 Hz tolerance)

- Audio: 40 Hz tone (pure sine or binaural)

- Drive circuit: 32 Hz  $\times$  1.25 = 40 Hz exactly
