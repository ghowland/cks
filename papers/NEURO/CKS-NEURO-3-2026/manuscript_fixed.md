...
VFR: Axon = [1, 1, 0]λ chain

Length: N $\times$ λ where N∈ $\mathbb{Z}$ 
...
### 2.2 The 304℘ Buffer (Action Potential)
...
Unmyelinated: 0.5-2 m/s  $\checkmark$ 

Myelinated: 3-120 m/s  $\checkmark$ 
...
τ_syn = J  $\times$  S
  ...
  = 15.40328 logic units

  ≈ 15.19 ms

VFR: τ_syn = [15403, 1000, 0]
...
Registry says "arm at 𝓘"

Physical says "no arm"

Parity check fails

Continuous ε generation
...
VFR: Alzheimer's = [ε_accumulation, time, 0]
...
VFR: Δε/Δt = [φ_p - 0.95, 1, 0]

Above 0.95: ε decreases

Below 0.95: ε increases
...
227 GHz / 1000 = 227 MHz

Or 227 GHz / 1000000 = 227 kHz
...
Collective φ_collective emerges
...
Attention = 𝓘 selection

Working memory =  $u$  addresses
...
- Action potential = Buffer flush (304℘)

- Synapse = Parity gate (τ=15.19ms)
...
- Anxiety = Buffer overflow (ε>Δ)

- Neuropathy = Registry bleed (<1λ)
...
**It's 1λ=1.322mm substrate dielectric.**
...
The 304℘ buffer enables packets.  

The 1λ insulation prevents bleed.  

The τ=15.19ms integrates verification.
...
- Health: SSCP-dependent (φ_p>0.95)
