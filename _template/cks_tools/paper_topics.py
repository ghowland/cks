TOPICS = [
    {"MATH": {
        "title": "The Foundation Stack (Core Pillars)",
        "subtitle": "The mathematical and physical origin points of the CKS framework. All application papers derive from these.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"QM": {
        "title": "Core Physics Layer",
        "subtitle": "Direct derivations of fundamental physics from the mathematical foundation.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"SM": {
        "title": "Core Physics Layer",
        "subtitle": "Direct derivations of fundamental physics from the mathematical foundation.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-QM-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"GR": {
        "title": "Core Physics Layer",
        "subtitle": "Direct derivations of fundamental physics from the mathematical foundation.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-QM-1-2026] → [@CKS-SM-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"TEST": {
        "title": "Experimental Falsification",
        "subtitle": "The definitive test protocol.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"DWDM": {
        "title": "Telecommunications & Photonics",
        "subtitle": "Optical communications and DWDM infrastructure.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-DWDM-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"COS": {
        "title": "Cosmology & Astronomy",
        "subtitle": "Large-scale structure and celestial mechanics.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"LEX": {
        "title": "Lexicon", 
        "subtitle": "Definitions and terminology for CKS.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"LOGI": {
        "title": "Logismos", 
        "subtitle": "Logismos Q calculus.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"OMNI": {
        "title": "Omni", 
        "subtitle": "Cross-domain synthesis and general studies.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"PHYS": {
        "title": "K-Space Physics", 
        "subtitle": "K-Space applied physical mechanics.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"BIO": {
        "title": "Biology & Life Sciences",
        "subtitle": "The human organism and biological systems as phase-coherent structures.",
         "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-BIO-1-2026]{{registry_last}} → {{registry}}",
   }},
    {"BODY": {
        "title": "Movement & Body Mechanics",
        "subtitle": "Physical movement, proprioception, and somatic systems.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-BODY-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"COG": {
        "title": "Cognition & Consciousness",
        "subtitle": "The mind, intelligence, and thought as substrate phenomena.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"NEURO": {
        "title": "Neuroscience",
        "subtitle": "Neural computation and brain architecture.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-NEURO-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"SENS": {
        "title": "Sensory Systems",
        "subtitle": "Perception and sensory processing.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-SENS-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"MED": {
        "title": "Medical Applications",
        "subtitle": "Clinical interventions and therapeutic protocols.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-MED-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"AI": {
        "title": "Computing & AI",
        "subtitle": "Hardware architecture and substrate programming.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-AI-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"MAT": {
        "title": "Materials Science",
        "subtitle": "Phase-engineered materials and metamaterials.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-MAT-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"SEMI": {
        "title": "Semiconductors",
        "subtitle": "Advanced semiconductor engineering.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"ENG": {
        "title": "Engineering & Mechanics",
        "subtitle": "Civil engineering, architecture, and mechanical systems.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-ENG-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"FLOW": {
        "title": "Fluid Dynamics",
        "subtitle": "Flow control and turbulence suppression.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"ENV": {
        "title": "Environment & Infrastructure",
        "subtitle": "Climate, lighting, water, and agricultural systems.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-ENV-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"SOC": {
        "title": "Social Systems",
        "subtitle": "Collective intelligence and organizational coherence.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-SOC-1-2026]{{registry_last}} → {{registry}}",
    }},
    {"LANG": {
        "title": "Language & Communication",
        "subtitle": "Resonant syntax and phonemic systems.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"DATA": {
        "title": "Data & Information Theory",
        "subtitle": "Information mechanics and digital systems.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"ART": {
        "title": "Art & Aesthetics",
        "subtitle": "Creative applications and aesthetic theory.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"EDU": {
        "title": "Education",
        "subtitle": "Unified pedagogical framework.",
         "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-EDU-1-2026]{{registry_last}} → {{registry}}",
   }},
    {"RTRT": {
        "title": "Red-Team Red-Team",
        "subtitle": "Red Teaming papers.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"WUWU": {
        "title": "Woo Woo", 
        "subtitle": "Things traditionally considered meta-physics.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"ADHM": {
        "title": "ADHM", 
        "subtitle": "Ad-Hom.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026]{{registry_last}} → {{registry}}",
    }},
    {"DISC": {
        "title": "Discovery Process",
        "subtitle": "Documentation of the Human-AI collaboration.",
        "path": "[@CKS-0-2026] → [@CKS-MATH-0-2026] → [@CKS-MATH-1-2026] → [@CKS-MATH-10-2026] → [@CKS-MATH-104-2026] → [@CKS-DISC-1-2026]{{registry_last}} → {{registry}}",
    }},
]


